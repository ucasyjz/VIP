    ...
    def __init__(
        ...
        for _ in range(num_layers):
            if self.add_attention:
                ...
                # attentions_center.append(None)
                # attentions_surrounding.append(None)
...
class UNetMidBlock2DCrossAttn(nn.Module):
    def __init__(
        ...
        self.attentions = nn.ModuleList(attentions)
        self.attentions_center = nn.ModuleList(attentions_center)
        self.attentions_surrounding = nn.ModuleList(attentions_surrounding)
        self.resnets = nn.ModuleList(resnets)
    ...
    def forward(
        ...
        encoder_hidden_states_center: Optional[torch.FloatTensor] = None,
        encoder_hidden_states_surrounding: Optional[torch.FloatTensor] = None,
        ...
        for attn, attn_center, attn_surrounding, resnet in zip(self.attentions, self.attentions_center,self.attentions_surrounding,self.resnets[1:]):
            if self.training and self.gradient_checkpointing:
                ...
                ### center surrounding ###
                mask = torch.nn.functional.interpolate(mask,size=(hidden_states.shape[2], hidden_states.shape[3]))

                hidden_states_center = attn_center(
                        hidden_states,
                        encoder_hidden_states=encoder_hidden_states_center,
                        cross_attention_kwargs=cross_attention_kwargs,
                        attention_mask=attention_mask,
                        encoder_attention_mask=encoder_attention_mask,
                        return_dict=False,
                    )[0]
                hidden_states_center = (mask != 1) * hidden_states_center
                hidden_states_surrouding = attn_surrounding(
                        hidden_states,
                        encoder_hidden_states=encoder_hidden_states_surrounding,
                        cross_attention_kwargs=cross_attention_kwargs,
                        attention_mask=attention_mask,
                        encoder_attention_mask=encoder_attention_mask,
                        return_dict=False,
                    )[0]
                hidden_states_surrouding = (mask == 1) * hidden_states_surrouding
                enhanced_hidden_states = hidden_states_center + hidden_states_surrouding
                a = nn.Parameter(torch.randn(1, 1)).to(device=hidden_states.device)
                hidden_states = (1 - a) * hidden_states_all + a * enhanced_hidden_states
...
class CrossAttnDownBlock2D(nn.Module):
    ...
    def forward(
        ...
        blocks = list(zip(self.resnets, self.attentions, self.attentions_center,self.attentions_surrounding))

        for i, (resnet, attn,attn_center,attn_surrounding) in enumerate(blocks):
            if self.training and self.gradient_checkpointing:
                ...
                ##### center surrounding ###
                mask = torch.nn.functional.interpolate(mask,size=(hidden_states.shape[2], hidden_states.shape[3]))
                # print(encoder_hidden_states_center.shape)
                hidden_states_center = attn_center(
                        hidden_states,
                        encoder_hidden_states=encoder_hidden_states_center,
                        cross_attention_kwargs=cross_attention_kwargs,
...
class CrossAttnUpBlock2D(nn.Module):
    ...
    def forward(
        ...
        for resnet, attn, attn_center,attn_surrounding in zip(self.resnets, self.attentions,self.attentions_center,self.attentions_surrounding):
            # pop res hidden states
            res_hidden_states = res_hidden_states_tuple[-1]
            res_hidden_states_tuple = res_hidden_states_tuple[:-1]
